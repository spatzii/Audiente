...
def tables_whole_day(csv, stations):
    ...
    return pd.concat([Channel(csv, station).get_rating_day() for station in stations],
                     axis=1).style.background_gradient().format(precision=2)


def graphs_whole_day(csv, stations):
    # Creates whole day chart
    return pd.concat([Channel(csv, station).get_graph_day() for station in stations], axis=1)


def tables_slot(csv, stations, timeslot):
    # Creates slot table
    return pd.concat([Channel(csv, station).get_rating_slot(timeslot) for station in stations],
                     axis=1).style.background_gradient().format(precision=2)


def graphs_slot(csv, stations, timeslot):
    # Creates slot chart
    return pd.concat([Channel(csv, station).get_graph_slot(timeslot) for station in stations], axis=1)
